...
> **Addendum (superseding "Non-goals (v1)" below):** after initial implementation, the requirements changed — users need their personal questions to follow them across sessions/devices, and the app owner needed a way to have their own generated questions land directly in canonical. This moved the app from a stateless/IndexedDB personal store to: **Google Sign-In (Google Identity Services, client-side) for every user**, with personal questions stored in **D1** in the same `questions` table via a nullable `owner_email` column (`NULL` = canonical, an email = personal to that user). The signed-in owner account (`CANONICAL_OWNER_EMAIL`, default `gdecugis@gmail.com`) is special-cased server-side: their generated questions are written with `owner_email = NULL`, i.e. straight into canonical, with no separate admin UI needed. See `functions/api/personal.js`, `functions/_googleAuth.js`, `auth.js`, and `d1/migrate-001-owner-email.sql`. Cloudflare Access, mentioned below as the original recommendation for gating a hypothetical admin write endpoint, was **not used** — Google auth is handled entirely client-side via Google Identity Services plus per-request ID token verification, since a real write endpoint (`/api/personal`) did end up getting built and needed real user identity, not just an admin gate.

> **Status: implemented and verified in preview, ready to merge.** Post-launch hardening added on top of the above: each generated batch is saved to D1 immediately (not accumulated and saved once at the end) so an interruption only risks the in-flight batch; a top-level React error boundary now shows a recoverable error instead of a blank page on any uncaught exception; the exam screen's bottom nav bar was `position: fixed` to the viewport (leaving a large empty gap under short question sets) and is now `position: sticky`; the results screen previously had no way back to the rest of the app and now does; and the generate screen offers 240/480 as additional bulk-generation sizes alongside 10/30/60/120.
